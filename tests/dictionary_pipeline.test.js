...
const dictMeta = {
  ...
  [dictRootPath + "/wty-en-de/index.json"]: {
    title: "wty-en-de",
    indexUrl:
      "https://huggingface.co/datasets/daxida/wty-release/resolve/main/latest/index/wty-en-de-index.json",
    downloadUrl:
      "https://huggingface.co/datasets/daxida/wty-release/resolve/main/latest/dict/en/de/wty-en-de.zip",
  },
  ...
  file: {
    exists(p) {
      return (
        ...
        Object.prototype.hasOwnProperty.call(dictMeta, p)
      );
    },
    read(p) {
      if (p === "/data/manifest.json")
        return JSON.stringify({ dictionaries: {}, disabled: {} });
...
assert(
  enPaths.length === 6,
  "English should keep every enabled dictionary active",
);
assert(
  enPaths.some((p) => p.endsWith("/Jitendex.org [2026-06-06]")),
  "English should not silently hide unknown/incompatible dictionaries",
);
const enCompatible = context.languageCompatibleDictionaries(en);
assert(
  enCompatible.length === 1,
  "English compatibility diagnostics should still identify English-looking dictionaries",
);
assert(
  enCompatible[0].name === "wty-en-de",
  "English compatibility diagnostics should identify wty-en-de",
);

const koPaths = context.activeDictionaryPaths(ko);
assert(
  koPaths.length === 6,
  "Korean should keep every enabled dictionary active",
);
assert(
  context.languageCompatibleDictionaries(ko)[0].name === "wty-ko-en",
  "Korean compatibility diagnostics should identify wty-ko-en",
);

const frPaths = context.activeDictionaryPaths(fr);
assert(
  frPaths.length === 6,
  "French should keep every enabled dictionary active",
);
assert(
  context.languageCompatibleDictionaries(fr)[0].name === "wty-fr-en",
  "French compatibility diagnostics should identify wty-fr-en",
);

const dePaths = context.activeDictionaryPaths(de);
assert(
  dePaths.length === 6,
  "German should keep every enabled dictionary active",
);
assert(
  context.languageCompatibleDictionaries(de)[0].name === "wty-de-en",
  "German compatibility diagnostics should identify wty-de-en",
);

const zhPaths = context.activeDictionaryPaths(zh);
assert(
  zhPaths.length === 6,
  "Chinese should keep every enabled dictionary active",
);
assert(
  context.languageCompatibleDictionaries(zh)[0].name === "cc-cedict-zh-en",
  "Chinese compatibility diagnostics should identify zh dictionaries",
);
...
assert(
  parsed.language === "en",
  "Worker fingerprint should include selected language",
);
assert(
  parsed.dictionaries.length === 12,
  "Worker fingerprint should include every supplied dictionary path",
);
assert(
  parsed.dictionaries[0] === enPaths[0],
  "Worker fingerprint should preserve dictionary order",
);
assert(
  parsed.dictionaries[enPaths.length] === koPaths[0],
  "Worker fingerprint should preserve repeated supplied order",
);
...
assert(
  orderedNames[0] === "wty-ko-en",
  "Profile dictionary order should put selected dictionaries first",
);
assert(
  orderedNames[1] === "Jitendex.org [2026-06-06]",
  "Profile dictionary order should preserve explicit order",
);
assert(
  context.disabledDictionaryMap()["wty-ko-en"] === true,
  "Profile disabled map should drive active dictionary state",
);
assert(
  context.profileSummaries()[0].id === "default",
  "Profile summaries should expose the default profile",
);
assert(
  context.profileSummaries()[0].locked === true,
  "Default profile should be marked locked",
);
assert(
  context.activeDictionaryProfile().preferences.lookupLanguage === "ko",
  "Profile preferences should preserve selected language",
);
assert(
  context.activeDictionaryProfile().preferences.scanLength === 24,
  "Profile preferences should fill default scan length",
);
assert(
  !Object.prototype.hasOwnProperty.call(
    context.activeDictionaryProfile().preferences,
    "unknownSetting",
  ),
  "Profile preferences should drop unknown settings",
);
context.file.read = originalRead;

assert(
  /No dictionaries installed\/enabled for English/.test(
    context.dictionarySetupMessage(en, []),
  ),
  "English setup message should be language-specific",
);
assert(
  /No dictionaries installed\/enabled for French/.test(
    context.dictionarySetupMessage(fr, []),
  ),
  "French setup message should be language-specific",
);
assert(
  /No dictionaries installed\/enabled for German/.test(
    context.dictionarySetupMessage(de, []),
  ),
  "German setup message should be language-specific",
);
assert(
  /No dictionaries installed\/enabled for Chinese/.test(
    context.dictionarySetupMessage(zh, []),
  ),
