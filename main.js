...
let lastSubtitleCueIdentity = null;
let lastNativeLayoutFingerprint = "";
let nativeLayoutStablePolls = 0;
let nativeSubtitlePropertyRebuildTimer = null;
let nativeSubtitleFontMetricCache = Object.create(null);
...
const IINATAN_ENGLISH_LANGUAGE = (() => {
  ...
  const YOMITAN_PREFIX_RULES =
    typeof IINATAN_ENGLISH_YOMITAN_PREFIX_RULES !== "undefined"
      ? IINATAN_ENGLISH_YOMITAN_PREFIX_RULES
      : [];
  const YOMITAN_DOUBLED_SUFFIX_RULES =
    typeof IINATAN_ENGLISH_YOMITAN_DOUBLED_SUFFIX_RULES !== "undefined"
      ? IINATAN_ENGLISH_YOMITAN_DOUBLED_SUFFIX_RULES
      : [];
  ...
  function lookupRequest(text, position) {
    ...
    const candidates = generateCandidates(displayText, {
      start: run.start,
      end: run.end,
    });
    ...
    for (let i = 0; i < YOMITAN_RULES.length; i++) {
      ...
      rules.push(
        deinflect.suffixInflection(
          ...
          rule[2],
          rule[3],
          "Yomitan " + (rule[4] || "French transform"),
        ),
      );
    }
    return rules;
...
const IINATAN_CHINESE_LANGUAGE = (() => {
  ...
  function dictionaryMatches(dict) {
    ...
    if (primary.indexOf("jitendex") >= 0) return false;
    return (
      /\b(chinese|mandarin|cantonese|hanzi|hanyu|zhongwen)\b/.test(primary) ||
  ...
  function lookupRequest(text, position, scanLength) {
    ...
    const chars = common.chars(normalized);
    const pos = common.clampPosition(position, chars.length);
    if (!chars.length || pos >= chars.length || !isHoverableChar(chars[pos]))
      return null;
    const maxChars = Math.max(
      1,
      Math.min(Number(scanLength) || 24, chars.length - pos),
    );
...
function normalizeManifestProfile(id, profile, manifest, existed) {
  ...
  const fallbackFromRoot =
    !existed &&
    (profileId === String(manifest.activeProfileId || DEFAULT_PROFILE_ID) ||
      profileId === DEFAULT_PROFILE_ID);
  const fallback = makeDefaultProfile(
    profileId,
    profileId === DEFAULT_PROFILE_ID ? "Default" : profileId,
  );
  return {
    id: profileId,
    name: String(source.name || fallback.name),
    dictionaryOrder: normalizeDictionaryOrder(
      Array.isArray(source.dictionaryOrder)
    ...
    disabled: normalizeDisabledMap(
      source.disabled && typeof source.disabled === "object"
        ? source.disabled
        : fallbackFromRoot
          ? manifest.disabled
          : {},
...
function normalizeManifestShape(manifest) {
  ...
  if (!profiles[out.activeProfileId])
    profiles[out.activeProfileId] = normalizeManifestProfile(
      out.activeProfileId,
      null,
      out,
      false,
  ...
  if (!profiles[DEFAULT_PROFILE_ID])
    profiles[DEFAULT_PROFILE_ID] = normalizeManifestProfile(
      DEFAULT_PROFILE_ID,
      null,
      out,
      false,
...
function activeDictionaryProfile(manifest) {
  ...
  return (
    ...
    makeDefaultProfile(DEFAULT_PROFILE_ID, "Default")
  );
}
...
function activeProfilePreferenceBool(key, fallback) {
  ...
  const fallbackValue = Object.prototype.hasOwnProperty.call(
    ...
    : fallback;
  try {
    return preferenceValueToBool(
      activeProfilePreferenceValue(preferenceKey, fallbackValue),
      fallbackValue,
    );
  } catch (_) {
...
function profileSummaries(manifest) {
  ...
  return Object.keys(normalized.profiles)
    .sort((a, b) => {
      ...
      return String(normalized.profiles[a].name || a).localeCompare(
        String(normalized.profiles[b].name || b),
      );
    })
    .map((id) => ({
      id,
...
function activeProfileDictionaryOrder(manifest) {
  return normalizeDictionaryOrder(
    activeDictionaryProfile(manifest).dictionaryOrder,
  );
}
...
function readDictionaryIndexMetadata(dictPath) {
  try {
    ...
    debugWarn(
      "Could not read dictionary index metadata for " +
        String(dictPath || "") +
        ": " +
        compactError(error),
    );
...
function normalizeDictionaryLanguage(value) {
  ...
  if (
    /^(zh|zho|chi|cmn|yue|wuu|hak|nan|chinese|mandarin|cantonese|hanzi|hanyu|zhongwen)$/.test(
      lang,
    )
  )
    return "zh";
...
function dictionaryListItemName(item) {
  const name = String(
    (item && item.filename) || filenameFromListPath(item && item.path) || "",
  ).trim();
...
function unorderedDictionaryDirs() {
  try {
    ...
    return file
      .list(dictRoot(), { includeSubDir: false })
        ...
        const manifestEntry =
          (manifest.dictionaries &&
            (manifest.dictionaries[name] ||
              manifest.dictionaries[meta.title])) ||
          {};
        return {
          name,
          path: dictPath,
...
function recommendedDictionariesByLanguage() {
  if (
    typeof RECOMMENDED_DICTIONARIES_BY_LANGUAGE !== "undefined" &&
    RECOMMENDED_DICTIONARIES_BY_LANGUAGE
  )
    return RECOMMENDED_DICTIONARIES_BY_LANGUAGE;
...
function allRecommendedDictionaryItems() {
  ...
  Object.keys(groups).forEach((language) => {
    if (Array.isArray(groups[language]))
      groups[language].forEach((item) => out.push(item));
  });
  return out;
}
...
function recommendedDictionaryUrlMatches(item, dict) {
  const installedUrl = normalizeRecommendedDictionaryUrl(
    dict && dict.downloadUrl,
  );
  if (!installedUrl) return false;
  const urls = [item && item.downloadUrl].concat(
    item && Array.isArray(item.downloadUrlAliases)
      ? item.downloadUrlAliases
      : [],
  );
  return urls.some(
    (url) => normalizeRecommendedDictionaryUrl(url) === installedUrl,
  );
}
...
function recommendedDictionaryTitleMatches(item, dict) {
  const prefixes =
    item && Array.isArray(item.titlePrefixes) && item.titlePrefixes.length
      ? item.titlePrefixes
      : [item && item.title];
  ...
  return prefixes.some(
    (prefix) =>
      recommendedDictionaryTitlePrefixMatches(title, prefix) ||
      recommendedDictionaryTitlePrefixMatches(name, prefix),
  );
}
function recommendedDictionaryMatchesInstalled(item, dict) {
  return !!(
    item &&
    dict &&
    (recommendedDictionaryUrlMatches(item, dict) ||
      recommendedDictionaryTitleMatches(item, dict))
  );
}
function recommendedDictionaryInstalledMatches(item, dicts) {
  ...
  (dicts || []).forEach((dict) => {
    ...
    const key = String(
      (dict && dict.path) || (dict && dict.name) || (dict && dict.title) || "",
    );
    if (key && seen[key]) return;
...
function disabledDictionaryMap(manifest) {
  return activeProfileDisabledMap(manifest || readManifest());
}
function dictionaryCompatibilityDetails(language, installed) {
  ...
  if (
    !lang ||
    lang.id === "ja" ||
    typeof lang.dictionaryMatches !== "function"
  ) {
  ...
  dicts.forEach((d) => {
    try {
      ...
      debugWarn(
        "Dictionary compatibility check failed language=" +
          String(lang.id || "") +
          " dict=" +
          String((d && d.name) || "") +
          ": " +
...
function dictionarySetupMessage(language, dicts) {
  ...
  if (lang.id === "ja")
    return "No dictionaries installed/enabled. Use Plugins -> iinatan -> Settings... to download recommended dictionaries.";
  return (
    "No dictionaries installed/enabled for " +
    label.replace(/\s*\(experimental\)\s*/i, "") +
    ". Import or enable a Yomitan dictionary ZIP."
  );
}
function dictionaryCompatibilityWarning(language, entries) {
  ...
  return (
    "No enabled dictionary is marked compatible with " +
    (lang.label || lang.id) +
    "; lookup will still try the enabled dictionaries."
  );
}
...
function setDictionaryEnabled(name, enabledNow) {
  const manifest = updateActiveProfile(readManifest(), (profile) => {
    ...
    if (enabledNow) delete profile.disabled[name];
    else profile.disabled[name] = true;
  });
...
function setDictionaryOrder(names) {
  ...
  const manifest = updateActiveProfile(readManifest(), (profile) => {
    profile.dictionaryOrder = dictionaryOrderWithInstalledNames(
      names,
      installedNames,
    );
  });
...
function removeDictionaryReferencesFromProfile(profile, removeMap) {
  ...
  profile.dictionaryOrder = normalizeDictionaryOrder(
    profile.dictionaryOrder,
  ).filter((name) => !removeMap[name]);
  profile.disabled = normalizeDisabledMap(profile.disabled);
...
function removeDictionaryReferencesFromManifest(manifest, names) {
  ...
  Object.keys(normalized.dictionaries || {}).forEach((key) => {
    ...
    if (removeMap[key] || removeMap[entry.title] || removeMap[entry.name])
      delete normalized.dictionaries[key];
  });
  Object.keys(normalized.profiles || {}).forEach((id) => {
    removeDictionaryReferencesFromProfile(normalized.profiles[id], removeMap);
  });
  normalized.dictionaryOrder = normalizeDictionaryOrder(
    normalized.dictionaryOrder,
  ).filter((name) => !removeMap[name]);
...
function replaceDictionaryReferencesInManifest(
  manifest,
  names,
  replacementName,
) {
  ...
  Object.keys(normalized.dictionaries || {}).forEach((key) => {
    ...
    if (
      key !== replacement &&
      (removeMap[key] || removeMap[entry.title] || removeMap[entry.name])
    )
      delete normalized.dictionaries[key];
  });
  Object.keys(normalized.profiles || {}).forEach((id) => {
    replaceDictionaryReferencesInProfile(
  ...
  normalized.dictionaryOrder = normalizeDictionaryOrder(
    normalized.dictionaryOrder,
  ).map((name) => (removeMap[name] && replacement ? replacement : name));
  normalized.dictionaryOrder = normalizeDictionaryOrder(
    normalized.dictionaryOrder,
  );
  normalized.disabled = normalizeDisabledMap(normalized.disabled);
...
function safeInstalledDictionaryPath(dictPath) {
  ...
  const relative =
    candidate.indexOf(root + "/") === 0 ? candidate.slice(root.length + 1) : "";
  const hasUnsafePart = relative
    .split("/")
    .some((part) => !part || part === "." || part === "..");
  if (
    !candidate ||
    candidate === root ||
    candidate.indexOf(root + "/") !== 0 ||
    hasUnsafePart
  ) {
    throw new Error(
      "Refusing to delete dictionary outside installed dictionary folder: " +
        candidate,
    );
  }
  return candidate;
...
function deleteDictionaryPathInBackground(dictPath, name) {
  execChecked("/bin/rm", ["-rf", "--", dictPath]).catch((error) => {
    debugWarn(
      "background cleanup failed for deleted dictionary " +
        String(name || "") +
        ": " +
        compactError(error),
    );
...
async function deleteDictionary(name) {
  ...
  if (!dict)
    throw new Error("Dictionary is not installed: " + String(name || ""));
  ...
  stopBackendWorker().catch((error) => {
    debugWarn(
      "dictionary delete could not stop worker before removing files: " +
        compactError(error),
    );
  });
...
async function replaceRecommendedDictionaryMatches(
  item,
  replacementName,
  matches,
) {
  ...
  const replacementPath = safeInstalledDictionaryPath(
    pathJoin(dictRoot(), replacement),
  );
  const seen = Object.create(null);
  const stale = [];
  ...
  await stopBackendWorker().catch((error) => {
    debugWarn(
      "recommended dictionary replacement could not stop worker before cleanup: " +
        compactError(error),
    );
  });
  await execChecked("/bin/mkdir", ["-p", deletedDictionaryRoot()]);
  for (const dict of stale) {
    const removedPath = deletedDictionaryPath(
      dict.name || dict.title || item.title,
    );
    await execChecked("/bin/mv", ["--", dict.path, removedPath]);
    deleteDictionaryPathInBackground(
      removedPath,
      dict.name || dict.title || item.title,
    );
  }
  writeManifest(
    replaceDictionaryReferencesInManifest(readManifest(), names, replacement),
  );
  rebuildMenu();
  if (typeof postDictionaryManagerState === "function")
    postDictionaryManagerState();
...
function createDictionaryProfile(name, sourceProfileId) {
  ...
  const source =
    manifest.profiles[
      String(sourceProfileId || manifest.activeProfileId || DEFAULT_PROFILE_ID)
    ] || activeDictionaryProfile(manifest);
  const sourcePreferences =
    source.id === manifest.activeProfileId
      ? currentProfilePreferenceSnapshot()
      : source.preferences;
  const profileName =
    String(name || nextProfileName(manifest.profiles)).trim() ||
    nextProfileName(manifest.profiles);
...
function renameDictionaryProfile(profileId, name) {
  const manifest = normalizeManifestShape(readManifest());
  const id = String(
    profileId || manifest.activeProfileId || DEFAULT_PROFILE_ID,
  );
  if (!manifest.profiles[id])
    throw new Error("Unknown dictionary profile: " + id);
...
function deleteDictionaryProfile(profileId) {
  ...
  if (!id || !manifest.profiles[id])
    throw new Error("Unknown dictionary profile: " + id);
  if (id === DEFAULT_PROFILE_ID)
    throw new Error("The first profile cannot be deleted.");
...
function updateDictionaryProfilePreferences(profileId, prefs) {
  ...
  if (!manifest.profiles[id])
    throw new Error("Unknown dictionary profile: " + id);
  const previous = normalizeProfilePreferences(
    manifest.profiles[id].preferences,
  );
  manifest.profiles[id].preferences = normalizeProfilePreferences(
    Object.assign({}, previous, prefs || {}),
  );
  writeManifest(manifest);
...
function setActiveDictionaryProfile(profileId) {
  ...
  const manifest = normalizeManifestShape(readManifest());
  if (!requested || !manifest.profiles[requested])
    throw new Error("Unknown dictionary profile: " + requested);
  const currentId = manifest.activeProfileId || DEFAULT_PROFILE_ID;
  if (manifest.profiles[currentId]) {
    manifest.profiles[currentId].preferences =
      currentProfilePreferenceSnapshot();
  }
  ...
  if (typeof postDictionaryManagerState === "function")
    postDictionaryManagerState();
  showOSD(
    "Switched iinatan profile: " + activeDictionaryProfile(normalized).name,
  );
}
function addSubMenuItemCompat(parent, item) {
  ...
  if (typeof parent.addSubMenuItem === "function")
    return parent.addSubMenuItem(item);
  if (typeof parent.addSubmenuItem === "function")
    return parent.addSubmenuItem(item);
  throw new Error(
    "This IINA build did not expose MenuItem.addSubMenuItem/addSubmenuItem",
  );
}
function addMenuItemSafe(item) {
  try {
    ...
    console.warn("Could not add menu item: " + compactError(error));
    return false;
  }
}
...
async function ensureBundledBackendInstalled() {
  ...
  if (!file.exists(bundledBinPath())) {
    ...
    throw new Error(
      "iinatan's lookup engine is missing. Install a packaged Apple Silicon build or run scripts/build_native_backend.sh while developing.",
    );
  }
  ...
  if (!result || result.status !== 0)
    throw new Error(
      "Could not install iinatan lookup engine: " +
  ...
  if (!moved || moved.status !== 0) {
    safeDelete(tmpPath);
    throw new Error(
      "Could not activate iinatan lookup engine: " +
...
function parseBackendJsonOutput(raw, stderr) {
  ...
  throw new Error(
    "Dictionary backend returned incomplete output. stdoutBytes=" +
      text.length +
      " stdoutPrefix=" +
      text.slice(0, 260) +
      " stderr=" +
      String(stderr || "").slice(0, 260),
  );
}
...
function backendCommandError(stage, args, result, parsed, parseError) {
  ...
  const status =
    result && result.status !== undefined ? Number(result.status) : null;
  ...
  try {
    ...
    debugVerbose(
      "backend exec done stage=" +
        ...
        " stderr=" +
        String(result.stderr || "").slice(0, 600),
    );
    ...
    try {
      ...
      if (result.status !== 0)
        throw backendCommandError(label, args, result, null, parseError);
      throw parseError;
    }
...
function titleFromImportResult(importResult, zipPath) {
  const raw = String(
    (importResult && (importResult.title || importResult.name)) || "",
  ).trim();
  if (raw) return raw;
  const fromZip = filenameFromPath(zipPath)
    .replace(/\.zip$/i, "")
    .trim();
...
function numericImportField(importResult, snakeName, camelName) {
  const value =
    importResult &&
    (importResult[snakeName] !== undefined
      ? importResult[snakeName]
      : importResult[camelName]);
  const n = Number(value);
...
function updateManifestAfterImport(importResult, zipPath) {
  ...
  const language = dictionaryLanguageFromMetadata(meta, {
    language:
      ...
      importResult.lang ||
      importResult.sourceLanguage ||
      importResult.targetLanguage,
  });
  let manifest = readManifest();
  const existing =
    (manifest.dictionaries && manifest.dictionaries[title]) || {};
...
async function importDictionaryZip(zipPath, existingTaskId) {
  ...
  const importArgs = [
    ...
    zipPath,
    dictRoot(),
    prefBool("lowRamImport", true) ? "--low-ram" : "--normal-ram",
  ];
  try {
    await ensureBundledBackendInstalled();
    if (!taskId)
      taskId = startOverlayTask(
        "dictionary-import",
        "Adding dictionary",
        "Preparing import...",
      );
    updateOverlayTask(taskId, {
      title: "Adding dictionary",
      message: "Importing dictionary...",
      detail: "Large dictionaries can take several minutes.",
    });
    const started = Date.now();
    const selected = selectedLanguageModule();
    debugLog(
      "dictionary import start language=" +
        String((selected && selected.id) || "") +
        " zipPath=" +
        JSON.stringify(String(zipPath || "")) +
        " zipExists=" +
        String(file.exists(zipPath)) +
        " zipFilename=" +
        JSON.stringify(filenameFromPath(zipPath)) +
        " args=" +
        JSON.stringify(importArgs),
    );
    const result = await runBackendJson(
      importArgs,
      Math.max(30000, prefNumber("importTimeoutMs", 1800000)),
    ...
    updateOverlayTask(taskId, {
      title: "Adding dictionary",
      message: "Saving dictionary list...",
      detail: "Refreshing installed dictionaries.",
    });
    try {
    ...
    updateOverlayTask(taskId, {
      title: "Adding dictionary",
      message: "Refreshing lookup worker...",
      detail: "The new dictionary will be available for hover popups.",
    });
    try {
      await stopBackendWorker();
    } catch (error) {
      debugWarn(
        "dictionary imported but worker refresh failed: " + compactError(error),
      );
      setOverlayStatus(
        "Dictionary imported, but worker restart failed. Restart iinatan or use Debug -> Restart Dictionary Lookup.",
        "error",
        12000,
      );
    }
    ...
    const msg =
      "Added " +
      titleFromImportResult(result, zipPath) +
      " (" +
      numericImportField(result, "term_count", "termCount") +
      " terms).";
    if (ownsTask)
      finishOverlayTask(
        taskId,
        true,
        msg,
        "Import took about " + elapsed + " seconds.",
      );
    else
      updateOverlayTask(taskId, {
        title: "Adding dictionary",
        message: msg,
        detail: "Import took about " + elapsed + " seconds.",
      });
    debugLog(
      "dictionary import complete title=" +
        JSON.stringify(titleFromImportResult(result, zipPath)) +
        " language=" +
        String(
          (
            readManifest().dictionaries[
              titleFromImportResult(result, zipPath)
            ] || {}
          ).language || "unknown",
        ) +
    ...
    const stage =
      error && (error.importStage || error.backendStage)
        ? String(error.importStage || error.backendStage)
        : "unknown";
    debugError(
      "dictionary import failed stage=" +
        stage +
        " selectedLanguage=" +
        String((selectedLanguageModule() || {}).id || "") +
        " zipPath=" +
        JSON.stringify(String(zipPath || "")) +
        " zipExists=" +
        String(zipPath ? file.exists(zipPath) : false) +
        " zipFilename=" +
        JSON.stringify(filenameFromPath(zipPath)) +
        " args=" +
        JSON.stringify(importArgs) +
        " backendExit=" +
        String(
          error && error.backendStatus !== undefined ? error.backendStatus : "",
        ) +
        " stdoutPrefix=" +
        JSON.stringify(
          String((error && error.backendStdoutPrefix) || "").slice(0, 500),
        ) +
        " stdoutSuffix=" +
        JSON.stringify(
          String((error && error.backendStdoutSuffix) || "").slice(0, 500),
        ) +
        " stderrPrefix=" +
        JSON.stringify(
          String((error && error.backendStderrPrefix) || "").slice(0, 500),
        ) +
        " stderrSuffix=" +
        JSON.stringify(
          String((error && error.backendStderrSuffix) || "").slice(0, 500),
        ) +
        " parsedJson=" +
        JSON.stringify((error && error.backendParsedJson) || null) +
    ...
    const userStage =
      stage === "manifest-update"
        ...
        : String(stage).indexOf("Dictionary import command") >= 0 ||
            stage === "backend-import"
          ? "Backend import command failed."
...
function isFilePickerCancelError(error) {
  ...
  return /cancel|cancelled|canceled|user abort|user-abort|user declined/.test(
    msg,
  );
}
...
async function chooseDictionaryZipPaths() {
  ...
  debugLog(
    "manual dictionary import: opening file chooser with zip filter and multi-select",
  );
  try {
    const selected = await resolveMaybePromise(
      utils.chooseFile("Choose Yomitan dictionary ZIPs", options),
    );
    const paths = normalizeChosenFilePaths(selected);
    debugLog(
      "manual dictionary import: filtered chooser returned count=" +
        paths.length +
        " sample=" +
    ...
    debugWarn(
      "manual dictionary import chooser with zip filter failed: " +
        compactError(error),
    );
  }

  debugLog(
    "manual dictionary import: opening fallback unfiltered file chooser",
  );
  try {
    const selected = await resolveMaybePromise(
      utils.chooseFile("Choose Yomitan dictionary ZIPs", {
        allowsMultipleSelection: true,
        allowMultipleSelection: true,
        multiple: true,
      }),
    ...
    const paths = normalizeChosenFilePaths(selected);
    debugLog(
      "manual dictionary import: unfiltered chooser returned count=" +
        paths.length +
        " sample=" +
        JSON.stringify(paths.slice(0, 5)),
    );
...
async function validateAndImportDictionaryZip(zipPath, source) {
  ...
  debugLog(
    "manual dictionary import validation source=" +
      String(source || "") +
      " ok=" +
      ...
      " path=" +
      JSON.stringify(String(validation.path || zipPath || "").slice(0, 260)),
  );
  if (!validation.ok) {
  ...
  debugLog(
    "manual dictionary import: importing validated zip path=" +
      JSON.stringify(validation.path),
  );
...
async function validateAndImportDictionaryZips(zipPaths, source) {
  ...
  for (let i = 0; i < paths.length; i++) {
    const result = await validateAndImportDictionaryZip(
      paths[i],
      label + "-" + String(i + 1),
    );
  ...
  if (imported.length > 1)
    notify("Imported " + imported.length + " dictionaries.", "info", 6500);
  return imported;
}
...
async function testFilePickerApiFromMenu() {
  ...
  const invalid = selected
    .map((p) =>
      dictionaryZipValidation(p, (candidate) => file.exists(candidate)),
    )
    .filter((result) => !result.ok);
  debugLog(
    "debug file picker test selected=" +
      JSON.stringify(selected.slice(0, 12)) +
      " invalid=" +
  ...
  if (!invalid.length)
    ...
    alert(
      "File picker returned invalid path(s): " +
        invalid.map((result) => result.message).join("; "),
    );
}
async function getRecommendedDictionaries(id) {
  ...
  if (!downloadUrl)
    throw new Error("Recommended dictionary has no download URL: " + title);
  let taskId = null;
  try {
    await ensureDataDirs();
    const previousMatches = recommendedDictionaryInstalledMatches(
      item,
      dictionaryDirs(),
    );
    taskId = startOverlayTask(
    ...
    const replaced = await replaceRecommendedDictionaryMatches(
      item,
      importedTitle,
      previousMatches,
    );
    const msg =
      (replaced.length ? "Updated " : "Added ") +
      importedTitle +
      " (" +
      (result.term_count || 0) +
      " terms).";
    finishOverlayTask(
      ...
      true,
      msg,
      "The dictionary is now available for lookup popups.",
    );
    return result;
  } catch (error) {
...
function backendLaunchPath() {
  return "/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/Applications/Xcode.app/Contents/Developer/usr/bin";
}
...
async function waitForWorkerReady(fingerprint, timeoutMs, generation) {
  ...
  while (Date.now() < deadline) {
    ...
    if (ready && (!last || ready.fingerprint !== last.fingerprint)) {
      debugWarn(
        "worker ready fingerprint mismatch expected=" +
          JSON.stringify(String(fingerprint || "")) +
          " actual=" +
          JSON.stringify(String(ready.fingerprint || "")) +
          " ready=" +
  ...
  try {
    if (file.exists(workerLogPath()))
      logHint =
        " Worker log: " + String(file.read(workerLogPath()) || "").slice(-1400);
  } catch (_) {}
  const lastState = last ? " Last ready state: " + JSON.stringify(last) : "";
  const mismatch =
    last && last.fingerprint !== fingerprint
      ? " Expected fingerprint: " + fingerprint
      : "";
  throw new Error(
    "Dictionary lookup did not become ready." + lastState + mismatch + logHint,
  );
}
async function ensureBackendWorker(dicts, language) {
  ...
  const advisory = dictionaryCompatibilityWarning(
    lang,
    activeDictionaryEntries(lang),
  );
  if (advisory) {
    debugWarn(advisory);
...
async function runWorkerQueueLookupDirect(
  ...
  const startedAt = Date.now();
  debugVerbose(
    "direct worker lookup write requestId=" +
      String(requestId || "") +
      " workerRequestId=" +
      id +
      " ensureMs=" +
    ...
    if (file.exists(resp)) {
      ...
      debugVerbose(
        "direct worker lookup done requestId=" +
          ...
          raw.length +
          " resultCount=" +
          (parsed && parsed.results ? parsed.results.length : "n/a"),
      );
      if (!parsed || parsed.ok === false)
        throw new Error(
          (parsed && parsed.error) || "Direct worker lookup failed",
        );
      return parsed;
    }
    if (file.exists(workerStopPath())) {
...
async function runWorkerLookupViaClientExec(
  ...
  const clientArgs = [
    ...
    String(maxResults),
    "--max-glossaries",
    String(maxGlossaries),
    "--scan-length",
    String(scanLength),
    "--mode",
    String(backendMode || "yomitan-japanese"),
    "--timeout-ms",
    String(timeout),
    "--",
    suffix,
  ...
  debugVerbose(
    "client exec lookup result requestId=" +
      String(requestId || "") +
      " elapsedMs=" +
      (Date.now() - lookupStartedAt) +
      " resultCount=" +
      (result && result.results ? result.results.length : "n/a"),
  );
  return result;
}
async function lookupViaWorker(
  ...
  const lang = language || selectedLanguageModule();
  debugVerbose(
    "lookupViaWorker begin requestId=" +
      String(requestId || "") +
      " language=" +
      lang.id +
      " suffix=" +
  ...
  if (prefBool("directWorkerIpc", true)) {
    try {
      ...
      debugWarn(
        "direct worker lookup failed requestId=" +
          String(requestId || "") +
          ": " +
          compactError(error),
      );
  ...
  const result = await runWorkerLookupViaClientExec(
    suffix,
    dicts,
    scanLength,
    maxResults,
  ...
  if (!result || result.ok === false)
    throw new Error((result && result.error) || "Worker client lookup failed");
  return result;
}
function glossaryTagsIndicateNonLemma(glossary) {
  const tags =
    String((glossary && glossary.definitionTags) || "") +
    " " +
    String((glossary && glossary.termTags) || "");
...
function lookupResultIsOnlyNonLemma(result) {
  ...
  for (let i = 0; i < results.length; i++) {
    const glossaries =
      results[i] && results[i].term && Array.isArray(results[i].term.glossaries)
  ...
  for (let i = 0; i < results.length && out.length < max; i++) {
    ...
    for (let g = 0; g < glossaries.length && out.length < max; g++) {
      ...
      for (let r = 0; r < parsed.length && out.length < max; r++) {
        ...
        if (!lemma || seen[lemma] || (alreadyTried && alreadyTried[lemma]))
          continue;
        seen[lemma] = true;
        out.push({
          text: lemma,
...
function appendLookupResultEntries(target, seen, candidateResult, limit) {
  const entries =
    candidateResult && Array.isArray(candidateResult.results)
...
async function lookupAtPosition(input, position, requestId) {
  ...
  debugVerbose(
    "lookupAtPosition request language=" +
      language.id +
      " pos=" +
      pos +
      " request=" +
  ...
  if (language.id === "fr")
    debugVerbose(
      "French deinflection rule loading upstreamRules=" +
        String(language.upstreamRuleCount || "unknown") +
        " mode=" +
        String(language.deinflectionMode || language.deinflection || ""),
    );
  if (language.id === "de")
    debugVerbose(
      "German deinflection rule loading mode=" +
        String(language.deinflectionMode || language.deinflection || ""),
    );
  if (language.id === "zh")
    debugVerbose(
      "Chinese parser/preprocessor selection mode=" +
        String((request && request.backendMode) || language.lookupMode || "") +
        " scanLength=" +
        String((request && request.scanLength) || scanLength),
    );
  ...
  const candidates =
    Array.isArray(request.candidates) && request.candidates.length
      ? request.candidates
          .filter((c) => c && c.text)
          .map((c) => Object.assign({}, c, { text: String(c.text || "") }))
      : [
          {
            text: request.lookupText,
            source: "lookupText",
  ...
  const effectiveScanLength = Math.max(
    1,
    Number(request.scanLength) || scanLength,
  );
  const backendMode =
    request.backendMode || language.backendMode || "yomitan-japanese";
  const languageCacheKey =
    request.cacheKey ||
    [
  ...
  if (lookupCache[key]) {
    debugVerbose(
      "lookupAtPosition cache hit lang=" +
        language.id +
        " pos=" +
        pos +
        " lookupText=" +
        JSON.stringify(lookupText) +
        " noResult=" +
        String(!!lookupCache[key].noResult) +
        " cacheKey=" +
        JSON.stringify(languageCacheKey),
    );
    return lookupCache[key];
  }
  debugVerbose(
    "lookupAtPosition cache miss lang=" +
      language.id +
      " mode=" +
      backendMode +
      " pos=" +
      ...
      JSON.stringify(
        candidates
          .map((c) => ({ text: c.text, source: c.source, reason: c.reason }))
          .slice(0, 24),
      ),
  );
  let result = null;
  ...
  async function followNonLemmaLemmaReferences(nonLemmaResult) {
    const lemmaCandidates = nonLemmaLemmaCandidates(
      nonLemmaResult,
      triedLookupTexts,
      maxResults,
    );
    for (
      let i = 0;
      i < lemmaCandidates.length && mergedEntries.length < maxResults;
      i++
    ) {
      const candidate = lemmaCandidates[i];
      triedLookupTexts[candidate.text] = true;
      const candidateScanLength = Math.max(
        1,
        charsOf(candidate.text).length || effectiveScanLength,
      );
      debugVerbose(
        "lookupAtPosition non-lemma lemma candidate language=" +
          language.id +
          " index=" +
          i +
          " text=" +
      ...
      const candidateResult = await lookupViaWorker(
        candidate.text,
        dicts,
        candidateScanLength,
        maxResults,
      ...
      debugVerbose(
        "lookupAtPosition non-lemma lemma result language=" +
          language.id +
          " index=" +
          i +
          " resultCount=" +
          (candidateResult && candidateResult.results
            ? candidateResult.results.length
            : 0),
      );
      if (
        !candidateResult ||
        !candidateResult.results ||
        !candidateResult.results.length ||
        lookupResultIsOnlyNonLemma(candidateResult)
      )
        continue;
      if (!candidateUsed) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const candidateScanLength = Math.max(
      ...
      Number(candidate.scanLength) ||
        charsOf(candidate.text).length ||
        effectiveScanLength,
    );
    debugVerbose(
      "lookupAtPosition candidate language=" +
        language.id +
        " index=" +
        i +
        " text=" +
    ...
    debugVerbose(
      "lookupAtPosition candidate result language=" +
        language.id +
        " index=" +
        i +
        " resultCount=" +
    ...
    if (
      ...
      if (lookupResultIsOnlyNonLemma(candidateResult)) {
        ...
        debugVerbose(
          "lookupAtPosition candidate non-lemma-only language=" +
            language.id +
            " text=" +
            JSON.stringify(candidate.text) +
            " continuing=true",
      ...
      debugVerbose(
        "lookupAtPosition candidate matched language=" +
          language.id +
          " text=" +
          JSON.stringify(candidate.text) +
          " mergedResultCount=" +
  ...
  if (candidateUsed) {
    ...
    debugVerbose(
      "lookupAtPosition using non-lemma fallback language=" +
        language.id +
        " text=" +
        JSON.stringify((candidateUsed && candidateUsed.text) || ""),
    );
  }
  if (!result) result = { ok: true, results: [] };
  if (!candidateUsed)
    debugVerbose(
      "lookupAtPosition no-result terminal language=" +
        language.id +
        " candidateCount=" +
        candidates.length +
        " reason=all-candidates-empty cacheKey=" +
        JSON.stringify(languageCacheKey) +
        " stopScanning=true candidates=" +
        candidates.map((c) => c.text).join(", "),
    );
  result.text = clean;
  result.position = pos;
...
function parseLookupPayload(payload) {
  ...
  if (typeof payload === "string") {
    ...
    if (pipe > 0 && /^\d/.test(raw.slice(0, pipe))) {
      return {
        requestId: raw.slice(0, pipe),
        position: Number(raw.slice(pipe + 1)),
        text: lastSubtitle || "",
      };
    }
    const parsed = JSON.parse(raw);
...
async function handleLookupAt(payload) {
  try {
    ...
    debugLog(
      "lookup payload parse failed: " +
        compactError(error) +
        " raw=" +
        String(payload).slice(0, 300),
    );
    postToOverlay("lookup-result", {
      requestId,
      ok: false,
      error: "Could not parse lookup payload: " + compactError(error),
    });
    return;
  }
  const requestId =
  ...
  const text =
    payload && typeof payload.text === "string"
      ...
      : lastSubtitle || "";
  const position =
    payload && payload.position !== undefined ? Number(payload.position) : 0;
  debugVerbose(
    "lookup-at received requestId=" +
      requestId +
      " pos=" +
      position +
      " textLen=" +
  ...
  postToOverlay("lookup-ack", {
    requestId,
    message: "Plugin received hover request.",
  });
  postToOverlay("lookup-status", {
    requestId,
    message: "Plugin received hover request.",
  });
  const inflightKey = cleanSubtitleText(text) + "\n" + position;
  try {
    if (!lookupInFlight[inflightKey])
      lookupInFlight[inflightKey] = lookupAtPosition(
        text,
        position,
        requestId,
      ).finally(() => {
        delete lookupInFlight[inflightKey];
      });
    const result = await lookupInFlight[inflightKey];
    debugVerbose(
      "lookup success requestId=" +
        requestId +
        " resultCount=" +
    ...
    debugLog(
      "lookup failed requestId=" + requestId + ": " + compactError(error),
    );
    postToOverlay("lookup-result", {
      requestId,
      ok: false,
...
function mockLongestRightwardLookup(text, position, dictionary, scanLength) {
  ...
  for (let len = maxChars; len >= 1; len--) {
    ...
    if (dictionary.indexOf(candidate) >= 0) {
      best = candidate;
      break;
    }
  }
...
function runLookupParserUnitTests() {
  ...
  for (const t of tests) {
    ...
    if (got !== t.expected)
      failures.push(
        t.text + " @" + t.pos + " expected " + t.expected + " got " + got,
      );
  }
  if (failures.length)
    alert("Lookup parser unit tests failed:\n" + failures.join("\n"));
  else
    alert(
      "Lookup parser unit tests passed: " +
        tests.length +
        "/" +
        tests.length +
        ".",
...
function runLanguageUnitTests() {
  ...
  check(
    ja.lookupMode === "yomitan-japanese",
    "Japanese should declare Yomitan/HoshiDicts mode",
  );
  ...
  check(
    zh.lookupMode === "prefix",
    "Chinese should declare prefix lookup mode",
  );
  check(ko.lookupMode === "exact", "Korean should declare exact lookup mode");
  check(
    typeof en.dictionaryMatches === "function",
    "English should expose dictionary compatibility checks",
  );
  check(
    typeof fr.dictionaryMatches === "function",
    "French should expose dictionary compatibility checks",
  );
  check(
    typeof de.dictionaryMatches === "function",
    "German should expose dictionary compatibility checks",
  );
  check(
    ja.lookupUnit === "character",
    "Japanese should use character lookup units",
  );
  ...
  check(
    zh.lookupUnit === "character",
    "Chinese should use character lookup units",
  );
  const englishText = "I am running fast";
  const enReq = en.lookupRequest(
    englishText,
    charsOf(englishText).indexOf("n"),
    24,
  );
  check(
    enReq && enReq.lookupText === "running",
    "English hover inside running should query running",
  );
  check(
    enReq && enReq.suffix !== "nning",
    "English should not query partial rightward suffixes",
  );
  check(
    enReq && enReq.backendMode === "exact",
    "English should use exact lookup",
  );
  check(
    enReq && enReq.cacheStrategy === "word-span",
    "English should use word-span cache semantics",
  );
  check(
    en.lookupRequest("RUNNING", 1, 24).lookupText === "running",
    "English should lowercase lookup text",
  );
  check(
    en.lookupRequest("Don't", 1, 24).lookupText === "don't",
    "English should preserve apostrophes while lowercasing",
  );
  const frReq = fr.lookupRequest("L’Homme", 2, 24);
  check(
    frReq && frReq.candidates.some((c) => c.text === "homme"),
    "French should generate elision-tail candidates",
  );
  const deReq = de.lookupRequest("Ich stehe morgen früh auf.", 5, 24);
  check(
    deReq && deReq.candidates.some((c) => c.text === "aufstehen"),
    "German should generate split-verb candidates",
  );
  check(
    fr
      .lookupRequest("mangent", 2, 24)
      .candidates.some((c) => c.text === "manger"),
    "French should load Yomitan present-indicative rules",
  );
  check(
    de
      .lookupRequest("Die Sammlung ist groß", 5, 24)
      .candidates.some((c) => c.text === "sammeln"),
    "German should load Yomitan -lung rules",
  );
  const zhReq = zh.lookupRequest("我喜欢中文", 0, 4);
  check(
    zhReq && zhReq.lookupText === "我喜欢中" && zhReq.backendMode === "prefix",
    "Chinese should use bounded prefix lookup",
  );
  const jaReq = ja.lookupRequest("魔法使い", 1, 24);
  check(
    jaReq && jaReq.lookupText === "法使い",
    "Japanese should keep rightward-prefix lookup",
  );
  check(
    jaReq && jaReq.cacheStrategy === "exact-position",
    "Japanese should keep exact-position cache semantics",
  );
  const koReq = ko.lookupRequest("한국어 공부", 1, 24);
  check(
    koReq && koReq.lookupText === "한국어",
    "Korean should query the contiguous Hangul run",
  );
  if (failures.length)
    alert("Language unit tests failed:\n" + failures.join("\n"));
...
function runSettingsAuditChecks() {
  ...
  check(
    Number.isFinite(Number(cfg.scanLength)) && cfg.scanLength >= 1,
    "scanLength should be numeric",
  );
  check(
    Number.isFinite(Number(cfg.maxEntries)) && cfg.maxEntries >= 1,
    "maxEntries should be numeric",
  );
  check(
    Number.isFinite(Number(cfg.maxGlossesPerEntry)) &&
      cfg.maxGlossesPerEntry >= 1,
    "maxGlossesPerEntry should be numeric",
  );
  check(
    Number.isFinite(Number(cfg.popupMaxHeightVh)) && cfg.popupMaxHeightVh >= 20,
    "popupMaxHeightVh should be sent to overlay",
  );
  check(
  ...
  check(
    ["dark", "light", ""].indexOf(cfg.popupThemeHint || "") >= 0,
    "popupThemeHint should resolve to a concrete hint when present",
  );
  check(
    cfg.etymologyCollapseDefault === "collapsed" ||
      cfg.etymologyCollapseDefault === "expanded",
    "etymologyCollapseDefault should be sent to overlay",
  );
  check(
    ["collapsed", "expanded", "inherit"].indexOf(
      cfg.wiktionaryEtymologyCollapseOverride,
    ) >= 0,
    "wiktionaryEtymologyCollapseOverride should be sent to overlay",
  );
  check(
    typeof cfg.customPopupCss === "string",
    "customPopupCss should be sent to overlay as a string",
  );
  check(
    typeof prefBool("directWorkerIpc", true) === "boolean",
    "directWorkerIpc should be boolean-readable",
  );
  check(
    typeof prefBool("fallbackToClientExec", true) === "boolean",
    "fallbackToClientExec should be boolean-readable",
  );
  check(
    Number.isFinite(prefNumber("directIpcPollMs", 2)),
    "directIpcPollMs should be numeric",
  );
  check(
    Number.isFinite(prefNumber("workerIdleSleepMs", 2)),
    "workerIdleSleepMs should be numeric",
  );
  if (failures.length)
    alert("Settings audit checks failed:\n" + failures.join("\n"));
...
function lookupBenchmarkCases() {
  ...
  sentences.forEach((sentence) => {
    ...
    for (let i = 0; i < chars.length; i++) {
      if (isJapaneseish(chars[i]))
        out.push({ sentence, position: i, char: chars[i] });
  ...
  const pick = (p) =>
    times.length
      ? times[
          Math.min(
            times.length - 1,
            Math.max(0, Math.floor((times.length - 1) * p)),
          )
        ]
      : 0;
...
async function runLookupPerformanceBenchmark() {
  try {
    debugLog(
      "BENCH starting lookup performance benchmark directIpc=" +
        String(prefBool("directWorkerIpc", true)) +
        " fallback=" +
        String(prefBool("fallbackToClientExec", true)),
    );
    ...
    for (let i = 0; i < Math.min(30, cases.length); i++) {
      ...
      const started = Date.now();
      try {
        const r = await lookupAtPosition(
          c.sentence,
          c.position,
          "bench-seq-" + i,
        );
        seqSamples.push({
          ...
          count: r && r.results ? r.results.length : 0,
        });
      } catch (error) {
        seqSamples.push({
          ok: false,
    ...
    const failed = seqSamples
      .concat(burstSamples)
      .filter((s) => !s.ok)
      .slice(0, 5);
    if (failed.length)
      debugWarn("BENCH failures sample=" + JSON.stringify(failed));
    showOSD(
      "iinatan benchmark done: seq median " +
        seqSummary.median +
        "ms, burst p95 " +
        burstSummary.p95 +
        "ms",
...
function showTaskPanelTest() {
  const id = startOverlayTask(
    "debug-task",
    "Task panel test",
    "This is where dictionary progress appears.",
  );
  updateOverlayTask(id, {
    title: "Task panel test",
    message: "Visible at top-center of the video overlay.",
    detail:
      "If you can see this panel, dictionary downloads and imports can show progress here.",
  });
  setTimeout(
    () =>
      finishOverlayTask(
        id,
        true,
        "Task panel test complete.",
        "The task panel will auto-hide shortly.",
      ),
    4500,
  );
  alert(
    "Task panel test started. If a player window is loaded, the panel should be visible on the video overlay.",
  );
}
...
function rebuildMenu() {
  ...
  try {
    ...
    addDebugMenuItem(debugMenu, "Run Lookup Performance Benchmark", () =>
      runLookupPerformanceBenchmark(),
    );
    addDebugMenuItem(debugMenu, "Run Lookup Parser Unit Tests", () =>
      runLookupParserUnitTests(),
    );
    addDebugMenuItem(debugMenu, "Run Language Unit Tests", () =>
      runLanguageUnitTests(),
    );
    addDebugMenuItem(debugMenu, "Run Settings Audit Checks", () =>
      runSettingsAuditChecks(),
    );
    ...
    addDebugMenuItem(debugMenu, "Show Task Panel Test", () =>
      showTaskPanelTest(),
    );
