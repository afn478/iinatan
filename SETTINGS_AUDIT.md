...
| `lookupLanguage` | `ja` | Language select | `src/languages/registry.js`, `src/main/10_subtitle_text_style.js`, `src/main/20_dictionary_manifest.js`, `src/main/30_backend_import_worker_lookup.js`, `src/main/60_overlay_lifecycle_toggle.js`, `src/overlay/overlay.js` | Japanese is default and keeps the existing HoshiDicts/Jitendex behavior. English and Korean are experimental exact/no-deinflection paths that filter enabled dictionaries by language-looking metadata/path. If no compatible dictionary is available or worker setup fails, native subtitles are restored/kept visible and a specific status is shown. Applied on next overlay config push, subtitle update, or hover lookup. |
