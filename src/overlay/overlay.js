(function () {
  ...
  const CONTROLLER_CONTEXT_DEFAULTS = {
    noPopup: {
      faceSouth: "lookup",
      faceEast: "resumePlayback",
      faceWest: "togglePause",
      leftShoulder: "subtitlePrevious",
      rightShoulder: "subtitleNext",
      dpadUp: "seekBackwardLong",
      dpadDown: "seekForwardLong",
      dpadLeft: "seekBackward",
      dpadRight: "seekForward",
    },
    withPopup: {
      faceSouth: "selectEntry",
      faceEast: "closePopup",
      faceWest: "togglePause",
      faceNorth: "audioMenu",
      leftShoulder: "subtitlePrevious",
      ...
      dpadUp: "scrollUp",
      dpadDown: "scrollDown",
      dpadLeft: "entryPrevious",
      dpadRight: "entryNext",
      leftTrigger: "ankiForceAdd",
      rightTrigger: "ankiPrimary",
    },
    audioList: {
      faceSouth: "audioActivate",
      faceEast: "closeAudioList",
      dpadUp: "audioPrevious",
      dpadDown: "audioNext",
      dpadLeft: "audioPreviousColumn",
      dpadRight: "audioNextColumn",
    },
  };
  const CONTROLLER_ACTION_ALIASES = {
    "subtitle-seek-previous": "subtitlePrevious",
    "subtitle-seek-next": "subtitleNext",
    "subtitle-previous": "subtitlePrevious",
    "subtitle-next": "subtitleNext",
    "seek-backward": "seekBackward",
    "seek-forward": "seekForward",
    "seek-backward-long": "seekBackwardLong",
    "seek-forward-long": "seekForwardLong",
    "toggle-pause": "togglePause",
    "resume-playback": "resumePlayback",
    "close-popup": "closePopup",
    "close-audio-list": "closeAudioList",
    "audio-menu": "audioMenu",
    "audio-activate": "audioActivate",
    "select-entry": "selectEntry",
    "frame-step-backward": "frameStepBackward",
    "frame-step-forward": "frameStepForward",
    "volume-down": "volumeDown",
    "volume-up": "volumeUp",
    "speed-down": "speedDown",
    "speed-up": "speedUp",
    "entry-previous": "entryPrevious",
    "entry-next": "entryNext",
    "scroll-up": "scrollUp",
    "scroll-down": "scrollDown",
    "popup-up": "scrollUp",
    "popup-down": "scrollDown",
    "popup-left": "entryPrevious",
    "popup-right": "entryNext",
    "popup-scroll-up": "scrollUp",
    "popup-scroll-down": "scrollDown",
    "play-audio": "playAudio",
    "audio-up": "audioPrevious",
    "audio-down": "audioNext",
    "audio-left": "audioPreviousColumn",
    "audio-right": "audioNextColumn",
    "anki-primary": "ankiPrimary",
    "anki-force-add": "ankiForceAdd",
  };
  ...
  function normalizeControllerAction(action) {
    if (action === null || action === false || action === "") return "";
    const value = String(action || "").trim();
    if (!value) return "";
    return CONTROLLER_ACTION_ALIASES[value] || value;
  }

  function normalizeControllerBindings(bindings) {
    ...
    const result = {};
    Object.keys(CONTROLLER_CONTEXT_DEFAULTS).forEach((context) => {
      const configured =
        input[context] && typeof input[context] === "object"
          ? input[context]
          : {};
      result[context] = Object.assign({}, CONTROLLER_CONTEXT_DEFAULTS[context]);
      Object.keys(CONTROLLER_CONTEXT_DEFAULTS[context]).forEach((button) => {
        const legacy = buttonAliases[button];
        const configuredButton = Object.prototype.hasOwnProperty.call(
          configured,
          button,
        )
          ? button
          : legacy && Object.prototype.hasOwnProperty.call(configured, legacy)
            ? legacy
            : null;
        if (configuredButton)
          result[context][button] = normalizeControllerAction(
  ...
  function mergeControllerBindings(bindings) {
    const input = bindings && typeof bindings === "object" ? bindings : {};
    const result = {};
    Object.keys(CONTROLLER_CONTEXT_DEFAULTS).forEach((context) => {
      const configured =
        input[context] && typeof input[context] === "object"
          ? input[context]
          : {};
      result[context] = Object.assign({}, configured);
    });
    return result;
  }

  function controllerContext() {
    if (state.audioSourceMenu) return "audioList";
    return controllerPopupVisible() ? "withPopup" : "noPopup";
  }

  function controllerBindings() {
    return normalizeControllerBindings(
      state.config && state.config.controllerBindings,
    );
  }

  function controllerActionForButton(button, context) {
    ...
    const action = Object.prototype.hasOwnProperty.call(
      bindings[selectedContext],
      String(button || ""),
    )
      ? bindings[selectedContext][String(button || "")]
      : CONTROLLER_CONTEXT_DEFAULTS[selectedContext][String(button || "")];
    const normalized = normalizeControllerAction(action);
    // The popup editor's "lookup" option means opening the selected entry;
    // the no-popup context uses the same stored action to open a subtitle
    // lookup. Keep the persisted action vocabulary compact while honoring
    // the context-specific command.
    if (selectedContext === "withPopup" && normalized === "lookup")
      return "selectEntry";
    return normalized;
  }

  function canonicalControllerButtons(buttons) {
    const input = buttons && typeof buttons === "object" ? buttons : {};
    const value = (canonical, legacy) =>
      !!(input[canonical] || (legacy && input[legacy]));
    return {
      faceSouth: value("faceSouth", "primary"),
      faceEast: value("faceEast", "back"),
      faceWest: value("faceWest", "square"),
      faceNorth: value("faceNorth", "audio"),
  ...
  function applyConfig(config) {
    ...
    state.config.controllerBindings = mergeControllerBindings(
      state.config.controllerBindings,
    );
  ...
  function refreshNativeSubtitleCopyForFont(options) {
    ...
    const fingerprint = JSON.stringify([
      ...
      Array.isArray(value.fallbackRuns) ? value.fallbackRuns : [],
    ]);
  ...
  function nativeTextNodesWithOffsets(root) {
    const nodes = [];
    const visit = (node, offset) => {
      if (!node) return offset;
      const children = node.childNodes;
      if (!children || !children.length) {
        if (node.nodeType !== 3 && node.tagName !== "#text") return offset;
        const text = String(node.textContent || "");
        if (!text.length) return offset;
        nodes.push({ node, start: offset, end: offset + text.length });
        return offset + text.length;
      }
      let nextOffset = offset;
      for (let index = 0; index < children.length; index++)
        nextOffset = visit(children[index], nextOffset);
      return nextOffset;
    };
    visit(root, 0);
    return nodes;
  ...
  function nativeTextBoundary(nodes, offset, preferNext) {
    for (let index = 0; index < nodes.length; index++) {
      const item = nodes[index];
      if (offset < item.end || (preferNext && offset === item.start))
        return {
          node: item.node,
  ...
  function nativeTextRangeRects(copyElement, start, end) {
    if (!document.createRange || !copyElement) return [];
    const nodes = nativeTextNodesWithOffsets(copyElement);
    if (!nodes.length || !Number.isInteger(start) || !Number.isInteger(end))
      return [];
    const total = nodes[nodes.length - 1].end;
    if (start < 0 || end < start || end > total) return [];
    const startPoint = nativeTextBoundary(nodes, start, true);
    const endPoint = nativeTextBoundary(nodes, end, false);
    try {
  ...
  function nativeRangeRects(lookupSpans, start, end, copyElement) {
    ...
    const startUtf16 = Number(displayStart.startUtf16);
    const endUtf16 = Number(displayEnd.endUtf16);
    if (!Number.isInteger(startUtf16) || !Number.isInteger(endUtf16)) return [];
    return nativeTextRangeRects(target, startUtf16, endUtf16);
  }

  function nativeCopyLineRects(copyElement) {
    if (!copyElement) return [];
    return nativeTextRangeRects(
      copyElement,
      0,
  ...
  function nativeCopyRequiresAutomaticWrap(displayText) {
    if (!document.createRange || !nativeSubtitleCopyEl.textContent) return true;
  ...
  function releaseNativeSubtitleLocalFont() {
    ...
    if (
      ...
      return;
    const faceRecords = Array.isArray(current.faceRecords)
      ? current.faceRecords
      : current.face
        ? [{ face: current.face, added: current.added }]
        : [];
    faceRecords.forEach((record) => {
      if (!record || !record.added || !record.face) return;
      try {
  ...
  function nativeSubtitleFontFamilyForMeasurement(
    ...
    if (genericFamily && !fallbackRequired)
    ...
    if (
      ...
      nativeSubtitleLocalFont.fallbackFingerprint === fallbackFingerprint
    ) {
      return nativeSubtitleFontResolution(
        nativeSubtitleLocalFont,
        target,
        isCurrentCueGeneration,
      );
    ...
    releaseNativeSubtitleLocalFont();
    const generation = ++nativeSubtitleLocalFontGeneration;
    const alias = "iinatan-native-subtitle-font-" + generation;
    const faceRequests = [
      {
        name: family,
        alias,
    ...
    try {
      faceRequests.forEach((request) => {
        const source =
          ...
          '")';
        const face = new FontFaceConstructor(request.alias, source, {
    ...
    record.ready = Promise.all(
      faceRecords.map((faceRecord) => Promise.resolve(faceRecord.face.load())),
    ).then((loadedFaces) => {
      if (nativeSubtitleLocalFont !== record) return null;
      loadedFaces.forEach((loadedFace, index) => {
        const faceRecord = faceRecords[index];
        faceRecord.face = loadedFace || faceRecord.face;
        fontSet.add(faceRecord.face);
        faceRecord.added = true;
      });
      record.localFontVerified = true;
      return record;
    });
    return nativeSubtitleFontResolution(record, target, isCurrentCueGeneration);
  }

  function renderNativeSubtitleFallbackRuns(
    target,
    text,
    options,
    fontResolution,
    ...
    const value = options && typeof options === "object" ? options : {};
    const runs = Array.isArray(value.fallbackRuns) ? value.fallbackRuns : [];
    if (!runs.length) {
      target.textContent = displayText;
      return true;
    }
    // Inline spans keep the fallback face local to the missing glyph while
    // leaving the containing text in one UTF-16 coordinate space for Range.
    const aliases =
    ...
    const appendText = (valueToAppend) => {
      if (valueToAppend)
        target.appendChild(document.createTextNode(valueToAppend));
    };
    for (let index = 0; index < runs.length; index++) {
      const run = runs[index] || {};
      const absoluteStart = Number(run.startUtf16);
      const absoluteEnd = Number(run.endUtf16);
      if (
        !Number.isInteger(absoluteStart) ||
        !Number.isInteger(absoluteEnd) ||
        absoluteEnd <= absoluteStart
      )
        return false;
      const start = Math.max(0, absoluteStart - baseOffset);
      const end = Math.min(displayText.length, absoluteEnd - baseOffset);
      if (end <= 0 || start >= displayText.length) continue;
    ...
    appendText(displayText.slice(cursor));
    return true;
  }
        ...
        if (
          ...
          fontReadiness.push(
            nativeSubtitleFontFamilyForMeasurement(
              ...
              .then((fontLoad) => {
                if (!fontLoad) return;
                const localFontsVerified =
                  fontLoad.fontResolution.localFontVerified === true;
                const fontSetAccepted =
                  localFontsVerified ||
                  fontSet.check(
                    fontLoad.fontSpec,
                ...
                const fontFamilyAvailable =
                  localFontsVerified ||
                  (fontLoad.fontResolution.fallbackRequired !== true &&
                    nativeFontFamilyAvailable(
                      calculated.fontFamily,
                      String(block.displayText || ""),
                    ));
                const fallbackRunsRendered = renderNativeSubtitleFallbackRuns(
                  copy,
                  String(block.displayText || ""),
                  layout.options,
                  fontLoad.fontResolution,
                  Number(block.displayStartUtf16) || 0,
                );
                record.fontReady =
                  isCurrent() &&
                  fontSetAccepted &&
                  fontFamilyAvailable &&
                  fallbackRunsRendered;
              })
              .catch(() => {
                record.fontReady = false;
              }),
          ...
          layout.options &&
          Array.isArray(layout.options.fallbackRuns) &&
          layout.options.fallbackRuns.length
        )
          record.fontReady = false;
      });
    });
      ...
      Promise.resolve(fontSet.ready)
        ...
        .then((fontLoad) => {
          ...
          const localFontsVerified =
            fontLoad.fontResolution.localFontVerified === true;
          const fontSetAccepted =
            localFontsVerified || fontSet.check(fontLoad.fontSpec, displayText);
  ...
  function sendControllerVideoSeek(seconds) {
    ...
    reportControllerInput(
      "action",
      { action: "video-seek", seconds: amount, sent },
      "action:video-seek:" + String(amount),
    );
    return sent;
  }

  function sendControllerMpvCommand(command, args) {
    const payload = {
    ...
    reportControllerInput(
      "action",
      { action: "mpv-command", command: payload.command, sent },
      "action:mpv-command:" + payload.command + ":" + payload.args.join(","),
    );
    return sent;
  }
  ...
  function normalizeControllerGamepad(gamepad) {
    ...
    const recognized = mapping === "standard" || standardShape || genericShape;
    ...
    // Keep the old native names readable until every producer uses canonical names.
    buttons.primary = buttons.faceSouth;
    buttons.back = buttons.faceEast;
    buttons.square = buttons.faceWest;
    buttons.audio = buttons.faceNorth;
    return {
      buttons,
      axes: {
        leftY: axis(1),
  ...
  function runControllerAction(action, now) {
    switch (normalizeControllerAction(action)) {
      case "lookup":
        return openFirstControllerLookup();
      case "selectEntry":
        return selectLargestVisibleControllerEntry();
      case "closePopup":
        hidePopup();
        return true;
      case "closeAudioList":
        ...
        return true;
      case "togglePause":
        return controllerTogglePause();
      case "resumePlayback":
        return sendControllerResumePlayback();
      case "subtitlePrevious":
        return sendControllerSubtitleSeek(-1);
      case "subtitleNext":
        return sendControllerSubtitleSeek(1);
      case "seekBackward":
        return sendControllerVideoSeek(-5);
      case "seekForward":
        return sendControllerVideoSeek(5);
      case "seekBackwardLong":
        return sendControllerVideoSeek(-60);
      case "seekForwardLong":
        return sendControllerVideoSeek(60);
      case "frameStepBackward":
        return sendControllerMpvCommand("frame-back-step");
      case "frameStepForward":
        return sendControllerMpvCommand("frame-step");
      case "volumeDown":
        return sendControllerMpvCommand("add", ["volume", "-5"]);
      case "volumeUp":
        return sendControllerMpvCommand("add", ["volume", "5"]);
      case "speedDown":
        return sendControllerMpvCommand("multiply", ["speed", "0.9"]);
      case "speedUp":
        return sendControllerMpvCommand("multiply", ["speed", "1.1"]);
      case "audioMenu":
        return startControllerHold("audio-menu", now);
      case "audioActivate":
        return activateControllerAudioFocus();
      case "playAudio":
        return controllerPlaySelectedAudio();
      case "audioPrevious":
        return moveControllerAudioFocus(-1, 0);
      case "audioNext":
        return moveControllerAudioFocus(1, 0);
      case "audioPreviousColumn":
        return moveControllerAudioFocus(0, -1);
      case "audioNextColumn":
        return moveControllerAudioFocus(0, 1);
      case "scrollUp":
        return scrollControllerPopupNotch(-1);
      case "scrollDown":
        return scrollControllerPopupNotch(1);
      case "entryPrevious":
        return moveControllerEntry(-1);
      case "entryNext":
        return moveControllerEntry(1);
      case "ankiPrimary":
        return startControllerHold("anki-primary", now);
      case "ankiForceAdd":
        return startControllerHold("anki-force-add", now);
      case "none":
      case "":
        return false;
      default:
  ...
  function dispatchControllerButton(button, now) {
    const action = controllerActionForButton(button);
    const dispatched = runControllerAction(action, now);
    if (
      dispatched &&
      state.controller.hold &&
      (action === "audioMenu" ||
        action === "ankiPrimary" ||
        action === "ankiForceAdd")
    )
      state.controller.hold.button = button;
    return dispatched;
  }
  ...
  function processControllerSnapshot(snapshot, now, deltaMs) {
    ...
    const buttons = canonicalControllerButtons(snapshot.buttons);
    ...
    [
      ...
      if (pressedEdge(button)) dispatchControllerButton(button, now);
    });
    [
      "faceSouth",
      "faceEast",
  ...
  iina.onMessage("controller-state", (payload) => {
    ...
    state.controller.nativeSnapshot = payload.connected
      ? {
          id: String(payload.id || "macOS Controller"),
