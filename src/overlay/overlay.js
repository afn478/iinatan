(function () {
  ...
  function renderAnkiButtonHtml(contextId) {
    ...
    return (
      '<button type="button" class="anki-button" data-clickable="true" data-anki-context-id="' +
      escapeHtml(contextId) +
      '" data-anki-state="' +
      stateName +
      '" data-anki-action="' +
      action +
      '" data-anki-note-ids="[]" title="' +
      escapeHtml(title) +
      '" aria-label="' +
      escapeHtml(title) +
      '">' +
  ...
  function setNoteIdsForAnkiButton(button, noteIds) {
    ...
    button.dataset.ankiNoteIds = JSON.stringify(
  ...
  function setAnkiButtonState(button, status) {
    ...
    if (stateName === "duplicate" && duplicate) {
      ...
      if (config.duplicateMode === "allow") {
        ...
        button.title =
          "Duplicate found. Click to add anyway; right-click to open existing note.";
  ...
  function ankiContextForButton(button) {
    const contextId =
      button && button.dataset ? button.dataset.ankiContextId : "";
  ...
  function noteIdsForButton(button) {
    try {
      const ids = JSON.parse(
        String(
          button && button.dataset ? button.dataset.ankiNoteIds || "[]" : "[]",
        ),
      );
  ...
  function failPendingAnkiMessage(pending, message) {
    ...
    if (
      ...
      pending.button.dataset.ankiRequestId === pending.requestId
    ) {
      if (pending.type === "anki-card-status")
        setAnkiButtonState(pending.button, {
          state: "ready",
          duplicate: false,
          noteIds: [],
  ...
  function completeAnkiOpenAfterSend(pending) {
    ...
    if (
      ...
      setAnkiButtonState(pending.button, {
        state: "opened",
        noteIds: pending.payload && pending.payload.noteIds,
  ...
  function trackAnkiBridgeMessage(button, payload, type) {
    ...
    const retryIfUnacked = () => {
      ...
      pending.retries++;
      overlayDebug(
        "retrying Anki bridge message type=" +
          String(type) +
          " requestId=" +
          requestId,
      );
  ...
  function sendAnkiCardMessage(button, type) {
    ...
    if (type !== "anki-card-open") {
      button.dataset.ankiState =
        type === "anki-card-add" ? "adding" : "checking";
    }
    ...
    const payload = {
      ...
      duplicateKnown:
        button && button.dataset
          ? String(button.dataset.ankiDuplicateKnown || "")
          : "",
      at: Date.now(),
    };
    return trackAnkiBridgeMessage(button, payload, type);
  }
        ...
        if (button.dataset.ankiBound !== "true") {
          ...
          button.addEventListener("click", (event) => {
            ...
            if (
              ...
              setAnkiButtonState(button, {
                state: "disabled",
                message: "Configure Anki export in settings",
              });
              return;
            }
            const duplicate =
              button.dataset.ankiState === "duplicate" ||
              button.dataset.ankiDuplicateKnown === "duplicate";
            const hasDuplicateIds = noteIdsForButton(button).length > 0;
            const openAction =
              button.dataset.ankiAction === "open" ||
              (duplicate &&
                hasDuplicateIds &&
                config.duplicateMode !== "allow");
            const type = openAction ? "anki-card-open" : "anki-card-add";
  ...
  function visibleSubtitleRect() {
    ...
    if (
      subtitleEl.classList.contains("hidden") ||
      !rect.width ||
      !rect.height
    ) {
      return {
        top: window.innerHeight * 0.72,
        bottom: window.innerHeight * 0.92,
        left: 0,
  ...
  function placePopup(anchor) {
    ...
    const availableBelow = Math.max(
      0,
      window.innerHeight - sub.bottom - margin - gap,
    );
    let placeAbove = true;
    ...
    const overlaps = !(
      top + popupH <= sub.top - gap / 2 || top >= sub.bottom + gap / 2
    );
