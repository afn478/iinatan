  ...
  function hidePopup() {
    ...
    clearActiveMatch();
	  ...
	  function setNoteIdsForAnkiButton(button, noteIds) {
	    ...
	    button.dataset.ankiNoteIds = JSON.stringify(Array.isArray(noteIds) ? noteIds : []);
	  }
	  function setAnkiButtonState(button, status) {
	    ...
	    const statusNoteIds = Array.isArray(status && status.noteIds) ? status.noteIds : null;
	    const config = activeAnkiConfig();
	  ...
	  function trackAnkiBridgeMessage(button, payload, type) {
	    ...
	    pending.finalTimer = setTimeout(() => {
	      ...
	      failPendingAnkiMessage(pending, type === 'anki-card-status' ? '' : 'Anki request timed out');
	    }, finalDelay);
	    return send();
	  }
	      ...
	      popupEl.querySelectorAll('.anki-button').forEach(button => {
	        if (button.dataset.ankiBound !== 'true') {
	          ...
	          button.addEventListener('click', event => {
	            ...
	            const config = activeAnkiConfig();
	            if (button.dataset.ankiAction === 'disabled' || button.dataset.ankiState === 'disabled') {
	              setAnkiButtonState(button, { state: 'disabled', message: 'Configure Anki export in settings' });
	              return;
	            }
	            const duplicate = button.dataset.ankiState === 'duplicate' || button.dataset.ankiDuplicateKnown === 'duplicate';
	            const hasDuplicateIds = noteIdsForButton(button).length > 0;
	            const openAction = button.dataset.ankiAction === 'open' || (duplicate && hasDuplicateIds && config.duplicateMode !== 'allow');
	  ...
	  function renderPopupHead(heading, reading, secondaryText, audioData, ankiData) {
	    ...
	    const actionHtml = audioHtml || ankiHtml ? '<div class="head-actions">' + ankiHtml + audioHtml + '</div>' : '';
